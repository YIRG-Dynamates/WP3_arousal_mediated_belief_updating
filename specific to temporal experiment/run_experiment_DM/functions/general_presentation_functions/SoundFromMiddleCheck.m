...
if KbResponseFlag
    ...
    Screen('DrawText',P.win, myTextCentre, round(D.win_center_x-boundsTextCentre(3)/2), round(D.win_center_y-boundsTextCentre(4)/2), P.draw_color);
...
for i=1:3
    ...
    trials_cell{1}.x = locs(i); %in degree visual angle
    trials_cell{1}.SOA = 500;   %in ms (not used because there's only one sound, which is followed by the lead_out)
    S.timing.lead_out = 0.45;   %TEMP - not saved (in seconds)
