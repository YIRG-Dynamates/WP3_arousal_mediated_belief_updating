function createTrialsOverview_SAC_tempo(trials_cell,T_all)
...
%Check that SAC levels match, that the direction change after each cp, and that the location (ISI) changes follow the given velocities    
for i=1:50
    assert(trials_cell{i}.SAC == T_all.SAC(i),['SAC levels do not match for trial ' num2str(i)]);
    assert(all(trials_cell{i}.d(2:end) == sign(trials_cell{i}.v(2:end))),['Directions (speed-up / slow-down) do not match velocity (ISI change) signs for trial ' num2str(i)]);
    assert(all((diff(trials_cell{i}.x) - trials_cell{i}.v(2:end)) < 1e6),['Velocities (ISI changes) do not match location (ISI) changes for trial ' num2str(i)]);
    if numel(trials_cell{i}.x) > 2
        assert(all(logical(diff(trials_cell{i}.d(2:end))) == trials_cell{i}.cp(3:end)),['Direction (speed-up / slow-down) changes and change-points do not match for trial ' num2str(i)]);
    end
end
...
%Check the velocity (ISI change) per SAC level, separately for the short and long trials
vels_short = nan(5,4);
vels_long = nan(5,6);
for i=1:5
    i_SAC = (T_all.SAC == i);
...
disp('Last velocities (ISI changes) for short trials per SAC level (rows)'); disp(vels_short);
disp('Last velocities (ISI changes) for long trials per SAC level (rows)'); disp(vels_long); 

%Check the velocity (ISI change) per spatial location (ISI), separately for the short and long trials
vels_short = nan(5,4);
vels_long = nan(5,6);
locs_short = nan(5,4);
locs_long = nan(5,6);
...
disp(' '); 
disp('Last velocities (ISI changes) for short trials per ISI bin (rows)'); disp(vels_short);
disp('Accompanying last ISI for the above velocities (ISI changes) for short trials'); disp(locs_short);
disp(' ');
disp('Last velocities (ISI changes) for long trials per ISI bin (rows)'); disp(vels_long);
disp('Accompanying last ISI for the above velocities (ISI changes) for long trials'); disp(locs_long);
