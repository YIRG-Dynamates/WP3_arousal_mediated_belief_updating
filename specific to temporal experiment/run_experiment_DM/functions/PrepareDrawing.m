...
D.respButton_lineWidth = 1;
D.respText = {'Speed-up','Slow-down'};
maxHor = 0;
maxVer = 0;
for i=1:numel(D.respText)
    D.boundsRespText{i} = Screen(P.win, 'TextBounds', D.respText{i});   %[L,T,R,B] from [0,0]
    maxHor = max(maxHor,D.boundsRespText{i}(3));
    maxVer = max(maxVer,D.boundsRespText{i}(4));
end
halfHor = round(.5*(maxHor + .2*maxHor));
halfVer = round(.5*(maxVer + .5*maxVer));
confBox_default = [-halfHor, -halfVer, halfHor, halfVer];
D.respButton_CentreLocs{1} = [D.win_center_x, D.win_center_y - startButtonHeight_pix];
D.respButton_CentreLocs{2} = [D.win_center_x, D.win_center_y + startButtonHeight_pix];
D.respButton_Coords{1} = confBox_default + [D.respButton_CentreLocs{1}, D.respButton_CentreLocs{1}];
D.respButton_Coords{2} = confBox_default + [D.respButton_CentreLocs{2}, D.respButton_CentreLocs{2}];
% use as: Screen('FrameRect', P.win,  P.draw_color,  D.respButton_Coords{1} , D.respButton_lineWidth);
...
halfHor = round(.5*(maxHor + .2*maxHor));
halfVer = round(.5*(maxVer + .5*maxVer));
confBox_default = [-halfHor, -halfVer+100, halfHor, halfVer+100];
