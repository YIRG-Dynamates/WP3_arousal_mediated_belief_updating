%%% UNPACKING AND PREPROCESSING OF DATA - MOTION EXPERIMENT %%%
...
% personalized skripts extract_pupil_data_all_motion_RF and
...
extract_pupil_data_all_motion_RF

%% %%%%%%%%% STEP 2: MOTION PREPROCESS (done 12.07.2024) %%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = 1:numel(eye_data_all)
    if isempty(eye_data_all{j}.pupilSize)
        disp(j)
        sim_length = eye_data_all{j}.A_stim_times(end) + 2000;
        eye_data_all{j}.fix1_sacc2_blink3  = ones(sim_length,1)*3;
        eye_data_all{j}.pupilSize  = zeros(sim_length,1);
        eye_data_all{j}.posX  = zeros(sim_length,1);
        eye_data_all{j}.posY  = zeros(sim_length,1);

        eye_data_all{j}.PUPblinks  = ones(sim_length,1);
    end
end
...
%% SAVING (last save 12.07.2024)

save(fullfile(save_folder,'trials_cell_all_motion.mat'),'trials_cell_all','-v7.3');
save(fullfile(save_folder,'eye_data_all_motion.mat'),'eye_data_all','-v7.3');
...
%% SAVING (last saved 12.07.2024)

save(fullfile(save_folder,'preprocessed_eye_data_motion_version_12_07_2024.mat'),'preprocessed_eye_data','-v7.3');
...
%% SAVING AGAIN (last saved 12.07.2024)
