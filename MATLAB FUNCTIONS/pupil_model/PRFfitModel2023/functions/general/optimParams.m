function fit_struct = optimParams(fitResultsStruct)
...
LB = fitResultsStruct.settings.fit_settings.bounds_transf.LB;
UB = fitResultsStruct.settings.fit_settings.bounds_transf.UB;
PLB = fitResultsStruct.settings.fit_settings.bounds_transf.PLB;
PUB = fitResultsStruct.settings.fit_settings.bounds_transf.PUB;

%% Create anonymous function for input to BADS
LL_fun = @(params) compLLallTrials(params,fitResultsStruct);                                  %Returns a log-likelihood vector (one LL value for each response)

MLE_or_MAP = fitResultsStruct.settings.fit_settings.optim_MLE_or_MAP;
...
BADSoptions.TolMesh = fitResultsStruct.settings.fit_settings.optim_tol_mesh;                  %The mesh is scaled between the plausible bounds (PUB-PLB = 1). 
                                                                                            %When the values don't change more than TolMesh, then BADS calls it converged (BADS default = 1e-6).
num_attempts = fitResultsStruct.settings.fit_settings.optim_num_attempts;
num_grid_search = fitResultsStruct.settings.fit_settings.optim_num_grid;
...
fit_struct.fittedParams = transformParams(fit_struct.fittedParams,fitResultsStruct.settings.fit_settings.transforms,'trans2real');
