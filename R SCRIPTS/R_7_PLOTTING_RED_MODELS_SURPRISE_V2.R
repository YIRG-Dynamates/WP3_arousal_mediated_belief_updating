# this script is for plotting SURPRISAL differently than the other one :)
...
both_domains_reduced_without_intercept <- readRDS("C:/Users/rfleischmann/Desktop/DATA/RAW THINGS/motion+localization (WP3)/PROC DATA/REGMODELS/nointer_both_reduced_delta_surp.rds") #path at ARI
summary(both_domains_reduced_without_intercept)


#### QUANTILING ####

# define limits for quantiles, these quantiles are over ALL surp values
qq <- quantile(df_all$surp, probs = seq(0, 1, 0.1))

# add column for quantiles
for (i in 1:10){
  df_all$quantile[df_all$surp >= qq[i]] <- i
}
...
qq_motion <- quantile(df_all_motion$surp, probs = seq(0, 1, 0.1))
qq_tempo <- quantile(df_all_tempo$surp, probs = seq(0, 1, 0.1))


for (i in 1:10){
  df_all_motion$quantile_m[df_all_motion$surp >= qq_motion[i]] <- i
  df_all_tempo$quantile_t[df_all_tempo$surp >= qq_tempo[i]] <- i
}
...
surp_participant_m <- matrix(NA, ncol = length(quantiles), nrow = length(subjects_motion))
delta_participant_t <- matrix(NA, ncol = length(quantiles), nrow = 27)
surp_participant_t <- matrix(NA, ncol = length(quantiles), nrow = 27)
...
for (subject in subjects_motion){
  for (i in quantiles){
    ...
    surp_participant_m[subject-100, i] <- mean(df_all_motion$surp[
      df_all_motion$quantile == i & df_all_motion$sbj == subject
      ])
  }
}
...
for (subject in subjects_tempo){
  for (i in quantiles){
    ...
    surp_participant_t[subject-200, i] <- mean(df_all_tempo$surp[
      df_all_tempo$quantile == i & df_all_tempo$sbj == subject
      ])
  }
}
...
Avg_surp_m <- colMeans(surp_participant_m, na.rm = TRUE)
Avg_surp_t <- colMeans(surp_participant_t, na.rm = TRUE)
...
quantile_data_motion <- data.frame(quant_surp = c(1:10), Avg_delta = Avg_delta_m, SE_delta = SEM_delta_m, Avg_surp = Avg_surp_m)
quantile_data_tempo <- data.frame(quant_surp = c(1:10), Avg_delta = Avg_delta_t, SE_delta = SEM_delta_t, Avg_surp = Avg_surp_t)

#### EFFECTS FROM MODEL ####

fixed_effects <- fixef(both_domains_reduced_without_intercept)

# Create a dataset for plotting with 'surp' ranging from -3 to 3
plot_data <- data.frame(
  surp = seq(-3, 3, length.out = 100)  # Range of 'surp' from -3 to 3
)

# Calculate predicted values using the fixed effects
plot_data$predicted <- fixed_effects["(Intercept)"] + 
  fixed_effects["surp"] * plot_data$surp
...
X <- model.matrix(~ surp, data = plot_data)  # Design matrix
...
##### MAKING THE MAIN PLOT - surprise ######

Q1min <- 0.042

mainplot_surp <- ggplot()+
  
  # this geom object predicts with a different model this might be wrong double check this
  # geom_smooth(data = df_all, aes(x = surp, y = deltas),
  #             method = "lm", level=0.95 , fullrange = TRUE, size = 1) +
  
  geom_ribbon(data = plot_data, 
              aes(x = surp, y = predicted, ymin = ci_lower, ymax = ci_upper), fill = "lightblue", alpha = 0.5) +  # Shaded CI
  
  geom_line(data = plot_data, 
            aes(x = surp, y = predicted),
            color = "darkgrey", linewidth = 1) +  # Regression line
  
  #dashed lines to mark the surp quintiles
  ...
  geom_point(data = quantile_data_motion,
             aes (x = Avg_surp, y=Avg_delta), colour="blue", alpha=0.9, size=2.5) +
  
  geom_errorbar(data = quantile_data_motion, 
                aes (x = Avg_surp, ymin = Avg_delta - SE_delta, ymax = Avg_delta + SE_delta), width=0.1, colour="blue", alpha=0.9, size=1) +
  
  geom_point(data = quantile_data_tempo,
             aes (x = Avg_surp, y=Avg_delta), colour="red", alpha=0.9, size=2.5) +
  
  geom_errorbar(data = quantile_data_tempo, 
                aes (x = Avg_surp, ymin = Avg_delta - SE_delta, ymax = Avg_delta + SE_delta), width=0.1, colour= "red", alpha=0.9, size=1) +
  
  
  # # text quintile 1
  # annotation_custom(grob = textGrob("Q1", gp = gpar(col = "black", fontsize = 12),
  #                                   just = "centre"), xmin = Avg_surp[1] , xmax = Avg_surp[1] , ymin = Q1min, ymax = 0.08)  +
  # # text quintile 2
  # annotation_custom(grob = textGrob("Q2", gp = gpar(col = "black", fontsize = 12),
  #                                   just = "centre"), xmin = Avg_surp[2] , xmax = Avg_surp[2] , ymin = Q1min, ymax = 0.08)  +
  # # text quintile 3
  # annotation_custom(grob = textGrob("Q3", gp = gpar(col = "black", fontsize = 12),
  #                                   just = "centre"), xmin = Avg_surp[3] , xmax = Avg_surp[3] , ymin = Q1min, ymax = 0.08)  +
  # # text quintile 4
  # annotation_custom(grob = textGrob("Q4", gp = gpar(col = "black", fontsize = 12),
  #                                   just = "centre"), xmin = Avg_surp[4] , xmax = Avg_surp[4] , ymin = Q1min, ymax = 0.08)  +
  # # text quintile 5
  # annotation_custom(grob = textGrob("Q5", gp = gpar(col = "black", fontsize = 12),
  #                                   just = "centre"), xmin = Avg_surp[5] , xmax = Avg_surp[5] , ymin = Q1min, ymax = 0.08)  +
  
  coord_cartesian(ylim=c(-0.07, 0.09), xlim=c(-2, 2)) + 
  
  ylab("Pupil Dilation") +
  xlab("Modelled Surprisal") +
  theme(
...
### saving
ggsave("C:\\Users\\rfleischmann\\Desktop\\DATA\\RAW THINGS\\motion+localization (WP3)\\PROC DATA\\PLOTS\\CENTRAL_PLOT_SURPRISE_PUPILGAIN.svg",
       plot = mainplot_surp, width = 6.2, height = 4, dpi = 300, device = "svg")
ggsave("C:\\Users\\rfleischmann\\Desktop\\DATA\\RAW THINGS\\motion+localization (WP3)\\PROC DATA\\PLOTS\\CENTRAL_PLOT_SURPRISE_PUPILGAIN.svg.png", 
       plot = mainplot_surp, width = 6.2, height = 4, dpi = 300, device = "png")
