function MAA = psybayes_plot_DM(psy,refparams)
...
if numel(psy.sigma) > 1
    ...
    % Compute SD of the posterior
    y = (y.*psy.sigma)/sum(y.*psy.sigma);   %DM: create probability distribution but take into account that we compute the sqrt(mean(variance)) rather than mean(SD)
...
if numel(psy.lambda) > 1
    ...
    box off; set(gca,'TickDir','out');
    if ~isempty(psy.units.lambda); string = [' (' psy.units.lambda ')']; else string = []; end
