...
%Generate one block of 50 trials for the direction discrimination task. 
%Loads of hard coded stuff, but much more efficient and better balanced.

%The general approach is to divide the trial conditions based on the last
%stimuli (SAC level, , last direction, last velocity, last spatial loc) and
%then start randomly sampling the sequences backwards (towards first stim).
...
%Absolute velocities of the last stimulus are divided into 2 (for short <=5 trials) or 3 (for long >5 trials) bins: small-large evidence for the last velocity   
%SAC levels (prior strengths) are completely balanced, with an equal amount of left and right high and low evidence trials.   
