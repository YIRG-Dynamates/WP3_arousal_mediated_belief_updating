...
if ~(exist(OverviewFile, 'file') == 2)
    ...
    HRTF_files = {};
    MAA_results = {};
    task_list = {'MA1','MA2','MA3','MA4','LD1','MA5','LD2','LD3','LD4','LD5'};  
    completed_tasks = zeros(0,numel(task_list));
    save(OverviewFile,'experimenter_IDs','computer_IDs','sound_cards','screen_numbers','keyboard_layout_nrs','subject_IDs','HRTF_files','MAA_results','task_list','completed_tasks');          
else
    load(OverviewFile,'experimenter_IDs','computer_IDs','sound_cards','screen_numbers','keyboard_layout_nrs','subject_IDs','HRTF_files','MAA_results','task_list','completed_tasks');
end
...
if size(completed_tasks,1) < S.subject_Nr
    ...
    %check that there is a known MAA result for this subject
    if next_task_nr > 2
        if (size(MAA_results,1) >= S.subject_Nr) && ~isempty(MAA_results{S.subject_Nr,1})
            S.MAA = MAA_results{S.subject_Nr,1};
            if (round(S.MAA*10)/10) > 4.5
                next_task_nr = 2;
            end
        else
            next_task_nr = 2;   %if MAA result is not known for this participant, run the MAA task again
        end
    end
end
...
switch short_task_name(1:2)
    case 'MA'
        S.task_name = 'MAA';
	case 'CD'
        S.task_name = 'Continuous_detection';
    case 'LD'
...
%Check that some default MAA setting exists
task_nrs_that_require_MAA = ~strncmp('MA',task_list,2);
if (next_task_nr <= 2) && task_nrs_that_require_MAA(S.task_nr)
    if isfield(S,'MAA') && ((round(S.MAA*10)/10) > 4.5)
        warn_msg = 'MAA for this subject is larger than the maximally allowed 4.5°. Please retry the MA2 task. For now, MAA will be set to the maximum value of 4.5°';
    else
        warn_msg = 'MAA not known for this subject, but task selected where MAA is required (not recommended!). MAA will be set to a "default" value of 4.5 degrees';
    end
    S.MAA = 4.5;
    warndlg(warn_msg);
    warning(warn_msg);
...
available_keyboard_layouts = {'rotating wheel with F1-F4'; 'keyboard only: "~/^°,tab,caps,shift" and L/R arrows'};
available_keyboard_layout_nrs = [1 2];

%Check if the saved screen_number is available to set as default choice
keyboard_layout_nr_default = 1;
if (size(keyboard_layout_nrs,1) >= computer_nr) && ~isempty(keyboard_layout_nrs{computer_nr,1})
    if ismember(keyboard_layout_nrs{computer_nr,1},available_keyboard_layout_nrs)
    	keyboard_layout_nr_default = find(keyboard_layout_nrs{computer_nr,1} == available_keyboard_layout_nrs);
    end
end

%Let the user select the appropriate keyboard layout for the confidence responses  
[selected_option,continueBool] = OptionsDialog(available_keyboard_layouts,keyboard_layout_nr_default,'Confidence Keys','Select the keys to use for confidence responses');
if ~continueBool                          
    error('Program is aborted');                        %dialog window was closed
end
S.keyboard_layout_nr = available_keyboard_layout_nrs(selected_option);

%If this keyboard layout was not known yet, then remember it for this computer
if (size(keyboard_layout_nrs,1) < computer_nr) || isempty(keyboard_layout_nrs{computer_nr,1}) || ~strcmp(S.keyboard_layout_nr,keyboard_layout_nrs{computer_nr,1})
    keyboard_layout_nrs{computer_nr,1} = S.keyboard_layout_nr;
end
