function dir_exp_fitfun_1(subj_nr)
...
trials_cell = reshape(trials_cell,[4*50 1]);        %4 blocks of 50 trials
...
options_struct.fit_settings.fit_param_names = {'sd_sens','k_azimuth_sens','cp_hazard_rate','lapse_rate'}; %The names of the parameters to fit 
options_struct.fit_settings.fit_param_nrs_per_cond = {[1 2 3 4]};           %Each of the parameters is fit using trials from the first and only condition
...
save(fullfile(save_path,[subj_ID '_BdCPfitResults_1.mat']),'subj_ID','BdCPfitResults','-v7.3');

%Report computation time in command window
fprintf('Finished fit 1 and saved data for subject %i (%s),\nElapsed time (days hours:minutes:seconds) %s \n', ... 
