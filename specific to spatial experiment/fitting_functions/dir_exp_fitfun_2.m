function dir_exp_fitfun_2(subj_nr)
...
options_struct.param_settings.cp_hazard_rate = 1/5;                         %Assume full knowledge of the generative function and ensure Bayesian model
options_struct.param_settings.lapse_rate = 0.01;                            %Use default lapse rate
...
save(fullfile(save_path,[subj_ID '_BdCPfitResults_2.mat']),'subj_ID','BdCPfitResults','-v7.3');

%Report computation time in command window
fprintf('Finished fit 2 and saved data for subject %i (%s),\nElapsed time (days hours:minutes:seconds) %s \n', ... 
